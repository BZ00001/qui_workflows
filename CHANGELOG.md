# Changelog

All notable changes to this automations repository are documented here. This file is auto-generated by `qui-sync export`; entries are prepended on each export that produces a diff.

## 2026-06-28

### Added
- `delete-unregistered-torrents` (movies) — Delete: unregistered torrents

### Updated
- `tag-scan` (series) — Tag: scan
- `delete-unregistered-torrents-2` (series) — Delete: unregistered torrents
- `delete-problem-cross-seeds-2` (series) — Delete: problem cross-seeds
- `delete-seed-goals-met` (series) — Delete: seed goals met
- `delete-stalled-downloads-2` (series) — Delete stalled downloads
- `upload-limit-unlimited-for-tier1-tier2-and-tier3-ratio-2` (series) — Upload Limit: Unlimited for Tier 1, Tier2 and Tier3 < Ratio 2
- `upload-limit-tier3-ratio-2` (series) — Upload Limit: Tier 3 Ratio > 2
- `move-torrents-stuck-on-pre-import-category-to-post-import-category` (series) — Move: Torrents stuck on Pre-Import Category to Post-Import Category
- `force-recheck-incomplete-cross-seeds` (series) — Force recheck: incomplete cross-seeds
- `resume-stopped-cross-seeds-90pct` (series) — Resume: stopped cross-seeds (>90%)
- `delete-free-disk-space` (series) — Delete: Free disk space
- `delete-stalled-downloads` (movies) — Delete stalled downloads
- `upload-limit-unlimited-for-tier1-tier2-and-tier3-ratio-2` (movies) — Upload Limit: Unlimited for Tier 1, Tier2 and Tier3 < Ratio 2
- `upload-limit-tier3-ratio-2` (movies) — Upload Limit: Tier 3 Ratio > 2
- `move-torrents-stuck-on-pre-import-category-to-post-import-category` (movies) — Move: Torrents stuck on Pre-Import Category to Post-Import Category
- `force-recheck-stopped-cross-seeds-90pct` (movies) — Force recheck: incomplete cross-seeds
- `resume-stopped-cross-seeds-90pct` (movies) — Resume: stopped cross-seeds (>90%)
- `delete-free-disk-space` (movies) — Delete: Free disk space

### Archived
- `delete-unregistered-torrents` (series) — Delete: unregistered torrents (moved to `archive/series/`)
- `delete-stalled-downloads` (series) — Delete stalled downloads (moved to `archive/series/`)

## 2026-06-26

### Added
- `delete-problem-cross-seeds-2` (series) — Delete: problem cross-seeds
- `delete-problem-cross-seeds-2` (movies) — Delete: problem cross-seeds

### Updated
- `tag-not-hardlinked` (series) — Tag: not Hardlinked
- `delete-unregistered-torrents` (series) — Delete: unregistered torrents
- `delete-unregistered-torrents-2` (movies) — Delete: unregistered torrents

### Archived
- `tag-single-episodes` (series) — Tag: Single Episodes (moved to `archive/series/`)
- `tag-season-pack` (series) — Tag: Season Pack (moved to `archive/series/`)
- `tag-unmatched-series` (series) — Tag: Unmatched Series (moved to `archive/series/`)
- `delete-tracker-error` (series) — Delete: Tracker Error (moved to `archive/series/`)
- `delete-unregistered-torrents` (movies) — Delete: unregistered torrents (moved to `archive/movies/`)
- `delete-tracker-error` (movies) — Delete: Tracker Error (moved to `archive/movies/`)

## 2026-06-18

### Updated
- `delete-problem-cross-seeds-2` (series) — Delete: problem cross-seeds
- `delete-seed-goals-met` (series) — Delete: seed goals met
- `delete-stalled-downloads` (series) — Delete stalled downloads
- `delete-tracker-error` (series) — Delete: Tracker Error
- `upload-limit-unlimited-for-tier1-tier2-and-tier3-ratio-2` (series) — Upload Limit: Unlimited for Tier 1, Tier2 and Tier3 < Ratio 2
- `upload-limit-tier3-ratio-2` (series) — Upload Limit: Tier 3 Ratio > 2
- `move-torrents-stuck-on-pre-import-category-to-post-import-category` (series) — Move: Torrents stuck on Pre-Import Category to Post-Import Category
- `force-recheck-incomplete-cross-seeds` (series) — Force recheck: incomplete cross-seeds
- `resume-stopped-cross-seeds-90pct` (series) — Resume: stopped cross-seeds (>90%)
- `delete-free-disk-space` (series) — Delete: Free disk space
- `delete-problem-cross-seeds-2` (movies) — Delete: problem cross-seeds
- `delete-seed-goals-met` (movies) — Delete: seed goals met
- `delete-stalled-downloads` (movies) — Delete stalled downloads
- `delete-tracker-error` (movies) — Delete: Tracker Error
- `upload-limit-unlimited-for-tier1-tier2-and-tier3-ratio-2` (movies) — Upload Limit: Unlimited for Tier 1, Tier2 and Tier3 < Ratio 2
- `upload-limit-tier3-ratio-2` (movies) — Upload Limit: Tier 3 Ratio > 2
- `move-torrents-stuck-on-pre-import-category-to-post-import-category` (movies) — Move: Torrents stuck on Pre-Import Category to Post-Import Category
- `force-recheck-stopped-cross-seeds-90pct` (movies) — Force recheck: incomplete cross-seeds
- `resume-stopped-cross-seeds-90pct` (movies) — Resume: stopped cross-seeds (>90%)
- `delete-free-disk-space` (movies) — Delete: Free disk space

### Archived
- `delete-problem-cross-seeds` (series) — Delete: problem cross-seeds (moved to `archive/series/`)
- `delete-problem-cross-seeds` (movies) — Delete: problem cross-seeds (moved to `archive/movies/`)

## 2026-06-13

### Added
- `force-recheck-incomplete-cross-seeds` (series) — Force recheck: incomplete cross-seeds

### Updated
- `resume-stopped-cross-seeds-90pct` (series) — Resume: stopped cross-seeds (>90%)
- `delete-free-disk-space` (series) — Delete: Free disk space
- `resume-stopped-cross-seeds-90pct` (movies) — Resume: stopped cross-seeds (>90%)
- `delete-free-disk-space` (movies) — Delete: Free disk space

### Renamed
- `force-recheck-stopped-cross-seeds-90pct` (movies) — "Force recheck: stopped cross-seeds (<90%)" → "Force recheck: incomplete cross-seeds"

### Archived
- `force-recheck-stopped-cross-seeds-90pct` (series) — Force recheck: stopped cross-seeds (< 90%) (moved to `archive/series/`)

## 2026-05-28

Add Delete on Tracker Error

### Added
- `delete-tracker-error` (series) — Delete: Tracker Error
- `delete-tracker-error` (movies) — Delete: Tracker Error

### Updated
- `upload-limit-unlimited-for-tier1-tier2-and-tier3-ratio-2` (series) — Upload Limit: Unlimited for Tier 1, Tier2 and Tier3 < Ratio 2
- `upload-limit-tier3-ratio-2` (series) — Upload Limit: Tier 3 Ratio > 2
- `move-torrents-stuck-on-pre-import-category-to-post-import-category` (series) — Move: Torrents stuck on Pre-Import Category to Post-Import Category
- `force-recheck-stopped-cross-seeds-90pct` (series) — Force recheck: stopped cross-seeds (< 90%)
- `resume-stopped-cross-seeds-90pct` (series) — Resume: stopped cross-seeds (>90%)
- `delete-free-disk-space` (series) — Delete: Free disk space
- `upload-limit-unlimited-for-tier1-tier2-and-tier3-ratio-2` (movies) — Upload Limit: Unlimited for Tier 1, Tier2 and Tier3 < Ratio 2
- `upload-limit-tier3-ratio-2` (movies) — Upload Limit: Tier 3 Ratio > 2
- `move-torrents-stuck-on-pre-import-category-to-post-import-category` (movies) — Move: Torrents stuck on Pre-Import Category to Post-Import Category
- `force-recheck-stopped-cross-seeds-90pct` (movies) — Force recheck: stopped cross-seeds (<90%)
- `resume-stopped-cross-seeds-90pct` (movies) — Resume: stopped cross-seeds (>90%)
- `delete-free-disk-space` (movies) — Delete: Free disk space

## 2026-05-17

Removed the Tier remove actions. They turned out to be unnecessary.

### Updated
- `tag-tier1` (series) — Tag: Tier 1
- `tag-tier2` (series) — Tag: Tier 2
- `tag-tier3` (series) — Tag: Tier 3
- `tag-tier1` (movies) — Tag: Tier 1
- `tag-tier2` (movies) — Tag: Tier 2
- `tag-tier3` (movies) — Tag: Tier 3

## 2026-05-01

### Updated
- `tag-scan` (series) — Tag: scan
- `tag-scan` (movies) — Tag: scan

## 2026-04-19

### Updated
- `tag-not-hardlinked` (series) — Tag: not Hardlinked
- `tag-not-hardlinked` (movies) — Tag: not Hardlinked

## 2026-04-19

### Added
- `tag-tier1` (series) — Tag: Tier 1
- `tag-tier2` (series) — Tag: Tier 2
- `tag-tier3` (series) — Tag: Tier 3
- `tag-not-hardlinked` (series) — Tag: not Hardlinked
- `tag-single-episodes` (series) — Tag: Single Episodes
- `tag-season-pack` (series) — Tag: Season Pack
- `tag-unmatched-series` (series) — Tag: Unmatched Series
- `tag-scan` (series) — Tag: scan
- `delete-unregistered-torrents` (series) — Delete: unregistered torrents
- `delete-problem-cross-seeds` (series) — Delete: problem cross-seeds
- `delete-seed-goals-met` (series) — Delete: seed goals met
- `delete-stalled-downloads` (series) — Delete stalled downloads
- `upload-limit-unlimited-for-tier1-tier2-and-tier3-ratio-2` (series) — Upload Limit: Unlimited for Tier 1, Tier2 and Tier3 < Ratio 2
- `upload-limit-tier3-ratio-2` (series) — Upload Limit: Tier 3 Ratio > 2
- `move-torrents-stuck-on-pre-import-category-to-post-import-category` (series) — Move: Torrents stuck on Pre-Import Category to Post-Import Category
- `force-recheck-stopped-cross-seeds-90pct` (series) — Force recheck: stopped cross-seeds (< 90%)
- `resume-stopped-cross-seeds-90pct` (series) — Resume: stopped cross-seeds (>90%)
- `delete-free-disk-space` (series) — Delete: Free disk space
- `tag-tier1` (movies) — Tag: Tier 1
- `tag-tier2` (movies) — Tag: Tier 2
- `tag-tier3` (movies) — Tag: Tier 3
- `tag-not-hardlinked` (movies) — Tag: not Hardlinked
- `tag-scan` (movies) — Tag: scan
- `delete-unregistered-torrents` (movies) — Delete: unregistered torrents
- `delete-problem-cross-seeds` (movies) — Delete: problem cross-seeds
- `delete-seed-goals-met` (movies) — Delete: seed goals met
- `delete-stalled-downloads` (movies) — Delete stalled downloads
- `upload-limit-unlimited-for-tier1-tier2-and-tier3-ratio-2` (movies) — Upload Limit: Unlimited for Tier 1, Tier2 and Tier3 < Ratio 2
- `upload-limit-tier3-ratio-2` (movies) — Upload Limit: Tier 3 Ratio > 2
- `move-torrents-stuck-on-pre-import-category-to-post-import-category` (movies) — Move: Torrents stuck on Pre-Import Category to Post-Import Category
- `force-recheck-stopped-cross-seeds-90pct` (movies) — Force recheck: stopped cross-seeds (<90%)
- `resume-stopped-cross-seeds-90pct` (movies) — Resume: stopped cross-seeds (>90%)
- `delete-free-disk-space` (movies) — Delete: Free disk space

## 2026-04-19

### Updated
- `move-torrents-stuck-on-pre-import-category-to-post-import-category` (series) — Move: Torrents stuck on Pre-Import Category to Post-Import Category
- `move-torrents-stuck-on-pre-import-category-to-post-import-category` (movies) — Move: Torrents stuck on Pre-Import Category to Post-Import Category

## 2026-04-19

### Added
- `upload-limit-unlimited-for-tier1-tier2-and-tier3-ratio-2` (series) — Upload Limit: Unlimited for Tier 1, Tier2 and Tier3 < Ratio 2
- `upload-limit-tier3-ratio-2` (series) — Upload Limit: Tier 3 Ratio > 2
- `move-torrents-stuck-on-pre-import-category-to-post-import-category` (series) — Move: Torrents stuck on Pre-Import Category to Post-Import Category
- `force-recheck-stopped-cross-seeds-90pct` (series) — Force recheck: stopped cross-seeds (< 90%)
- `resume-stopped-cross-seeds-90pct` (series) — Resume: stopped cross-seeds (>90%)
- `upload-limit-unlimited-for-tier1-tier2-and-tier3-ratio-2` (movies) — Upload Limit: Unlimited for Tier 1, Tier2 and Tier3 < Ratio 2
- `upload-limit-tier3-ratio-2` (movies) — Upload Limit: Tier 3 Ratio > 2
- `move-torrents-stuck-on-pre-import-category-to-post-import-category` (movies) — Move: Torrents stuck on Pre-Import Category to Post-Import Category
- `force-recheck-stopped-cross-seeds-90pct` (movies) — Force recheck: stopped cross-seeds (<90%)
- `resume-stopped-cross-seeds-90pct` (movies) — Resume: stopped cross-seeds (>90%)

### Updated
- `tag-tier1` (series) — Tag: Tier 1
- `tag-tier2` (series) — Tag: Tier 2
- `tag-tier3` (series) — Tag: Tier 3
- `tag-not-hardlinked` (series) — Tag: not Hardlinked
- `tag-single-episodes` (series) — Tag: Single Episodes
- `tag-season-pack` (series) — Tag: Season Pack
- `tag-unmatched-series` (series) — Tag: Unmatched Series
- `tag-scan` (series) — Tag: scan
- `delete-unregistered-torrents` (series) — Delete: unregistered torrents
- `delete-problem-cross-seeds` (series) — Delete: problem cross-seeds
- `delete-seed-goals-met` (series) — Delete: seed goals met
- `delete-stalled-downloads` (series) — Delete stalled downloads
- `delete-free-disk-space` (series) — Delete: Free disk space
- `tag-tier1` (movies) — Tag: Tier 1
- `tag-tier2` (movies) — Tag: Tier 2
- `tag-tier3` (movies) — Tag: Tier 3
- `tag-not-hardlinked` (movies) — Tag: not Hardlinked
- `tag-scan` (movies) — Tag: scan
- `delete-unregistered-torrents` (movies) — Delete: unregistered torrents
- `delete-problem-cross-seeds` (movies) — Delete: problem cross-seeds
- `delete-seed-goals-met` (movies) — Delete: seed goals met
- `delete-stalled-downloads` (movies) — Delete stalled downloads
- `delete-free-disk-space` (movies) — Delete: Free disk space

